...
class RegistrationLoadTester:
    def __init__(self, base_url: str = "http://localhost:8000", test_data_dir: str = None):
        ...
        Args:
            ...
            test_data_dir: Directory containing test images (defaults to Aerovision-V1/data/labeled)
        """
    ...
    async def _single_request(self, session: aiohttp.ClientSession, image_base64: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with session.post(f"{self.base_url}/api/v1/registration", json=payload) as response:
    ...
    async def _run_concurrent_test(self, session: aiohttp.ClientSession, concurrent_users: int, duration_seconds: int = 30) -> Dict[str, Any]:
        ...
        async def continuous_requests():
            ...
            while time.time() < end_time:
                ...
                result = await self._single_request(session, image_base64)
    ...
    async def run_stepped_load_test(self, concurrency_levels: List[int] = [1, 2, 4, 8, 16], duration_per_level: int = 30):
        """Run stepped load test from low to high concurrency."""
        print("=" * 60)
        print("Starting Stepped Load Test for Registration OCR")
        print("=" * 60)
    ...
    def save_results(self, output_file: str = "registration_load_test_results.json"):
...
async def main():
    parser = argparse.ArgumentParser(description="Registration OCR API Load Tester")
    ...
    parser.add_argument("--data-dir", help="Test data directory with images")
    ...
    tester = RegistrationLoadTester(base_url=base_url, test_data_dir=args.data_dir)
    await tester.run_stepped_load_test(duration_per_level=args.duration)
    tester.save_results(f"registration_load_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
